...
void Builtins::InitializeIsolateDataTables(Isolate* isolate) {
  ...
  std::cout << "InitializeIsolateDataTables: Builtins count: " << std::dec
            << ToInt(Builtins::kLast)
            << " BuiltinsT0 count: " << ToInt(Builtins::kLastTier0)
            << std::endl;
  // The entry table.
  for (Builtin i = Builtins::kFirst; i <= Builtins::kLast; ++i) {
    ...
    if (i == Builtins::kFirst) {
      std::cout << "bt[" << ToInt(i) << "]: " << std::hex
                << embedded_data.InstructionStartOfBuiltin(i) << std::endl;
    }
  }
  ...
  for (Builtin i = Builtins::kFirst; i <= Builtins::kLastTier0; ++i) {
    ...
    if (i == Builtins::kFirst) {
      std::cout << "btt0et[" << ii << "]: " << std::hex
                << isolate_data->builtin_tier0_entry_table()[ii] << std::endl;
      std::cout << "btt0[" << ii << "]: " << std::hex
                << isolate_data->builtin_table()[ii] << std::endl;
    }
  }
}
